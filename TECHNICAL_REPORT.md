...
**Çevrimdışı Mod:** `sqflite` bağımlılığı mevcut olmakla birlikte, aktif bir çevrimdışı önbellekleme katmanı henüz uygulanmamıştır. Firestore çevrimdışı kalıcılık özelliği ve `sqflite` kombinasyonuyla ağ bağlantısı gerektirmeyen bir kullanım deneyimi sağlanabilir.
